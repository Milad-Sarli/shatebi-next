...
const ModernStatCard = ({
  ...
  <motion.div
    ...
    className="w-full"
  >
    <Card className="flex flex-col justify-between px-3 py-3 sm:px-6 sm:py-5 bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 rounded-xl sm:rounded-2xl shadow-md hover:shadow-lg transition-all duration-150 min-h-[80px] sm:min-h-[120px]">
      <div className="flex items-center justify-between w-full mb-1 sm:mb-2">
        <div
          className="flex items-center justify-center rounded-lg sm:rounded-xl shadow w-8 h-8 sm:w-12 sm:h-12"
          style={{
        ...
        <div className="text-xs font-semibold text-zinc-400 dark:text-zinc-500 text-right leading-tight max-w-[120px] sm:max-w-[150px] overflow-hidden sm:overflow-visible">
          <div 
            className={`whitespace-nowrap sm:whitespace-normal hover:animate-none ${
              stat.title.length > 20 ? 'animate-scroll-text' : ''
            }`}
          >
            {stat.title}
          </div>
      ...
      <div className="flex items-end justify-between w-full">
        <span className="text-lg sm:text-2xl font-extrabold text-zinc-900 dark:text-zinc-100">{stat.value}</span>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-2 sm:gap-4 mb-1 sm:mb-2">
          <div className="flex items-center gap-2 sm:gap-3">
            <div className="rounded-full bg-blue-100 p-1.5 sm:p-2">
              <GraduationCap className="h-4 w-4 sm:h-6 sm:w-6 text-blue-600" />
            </div>
            <div>
              <div className="text-xs text-zinc-500 dark:text-zinc-400 font-semibold">نام دارالقرآن</div>
              <div className="text-sm sm:text-lg font-bold text-zinc-800 dark:text-zinc-200 truncate">{user?.tenant?.title}</div>
            </div>
          </div>
