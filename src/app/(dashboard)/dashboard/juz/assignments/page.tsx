...
import { BookMarked, Save, Loader2, Download, CheckCircle2, Clock } from "lucide-react";
...
export default function JuzAssignmentsPage() {
  ...
  const days = useMemo(() => {
    ...
    while (current <= toGreg) {
      ...
      if (dow !== null) {
        result.push({ dateStr: formatJalaliDate(current), greg: new Date(current), dayOfWeek: dow });
      }
  ...
  function setCellValue(studentId: number, dateStr: string, juz: number[]) {
    setValues(prev => {
      const key = `${studentId}`;
  ...
  async function handleLoad() {
    ...
    try {
      ...
      const allAssignments = (Array.isArray(assignmentsRes.data) ? assignmentsRes.data : assignmentsRes.data?.data || []) as Array<{ student_id: number; juz_number: number; day_of_week: number }>;
      const allLogs = (Array.isArray(logsRes.data) ? logsRes.data : logsRes.data?.data || []) as Array<{ student_id: number; juz_number: number; read_date: string }>;
      ...
      for (const studentId of selectedStudentIds) {
        ...
        for (const day of days) {
          const matchingAssignments = allAssignments.filter(
            a => a.student_id === Number(studentId) && a.day_of_week === day.dayOfWeek
          );
          if (matchingAssignments.length > 0) {
            if (!newValues[sid]) newValues[sid] = {};
            newValues[sid][day.dateStr] = matchingAssignments.map(a => a.juz_number);
          }

          const dateStr = day.greg.toISOString().slice(0, 10);
          const dayLogs = allLogs.filter(
            l => l.student_id === Number(studentId) && l.read_date?.startsWith(dateStr)
          );
          if (dayLogs.length > 0) {
      ...
      toast({ title: "خطا", description: err instanceof Error ? err.message : "خطا در بارگذاری", type: "destructive" });
    } finally {
  ...
  async function handleSave() {
    const assignments: Array<{ student_id: number; juz_number: number; day_of_week: number }> = [];

    for (const studentId of selectedStudentIds) {
      for (const day of days) {
        const juzes = getCellValue(Number(studentId), day.dateStr);
    ...
    if (assignments.length === 0) {
      toast({ title: "خطا", description: "حداقل یک جزء برای یک روز انتخاب کنید", type: "destructive" });
      return;
    }

    setSaving(true);
    try {
      const res = await JuzService.bulkCreateAssignments(assignments);
      if (res.status === "success") {
        toast({ title: "موفق", description: `${assignments.length} تکلیف با موفقیت ذخیره شد` });
        setValues({});
      }
    } catch (err: unknown) {
      toast({ title: "خطا", description: err instanceof Error ? err.message : "خطا در ذخیره", type: "destructive" });
    } finally {
      setSaving(false);
    }
  }
    ...
    <PageTransition>
      <div className="flex flex-col gap-6 w-full max-w-7xl mx-auto px-2 sm:px-4 py-3 sm:py-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-base sm:text-lg font-bold text-zinc-800 dark:text-zinc-200">تنظیم برنامه جزء خوانی</div>
            <div className="text-sm text-zinc-500 dark:text-zinc-400">انتخاب قرآن آموزان و تعیین جزء به صورت روزانه</div>
          </div>
        </div>
        ...
        <Card className="p-4 sm:p-6 bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 rounded-xl shadow-md">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="md:col-span-2">
              <label className="block text-sm font-semibold text-zinc-700 dark:text-zinc-300 mb-1.5 text-right">قرآن آموزان</label>
              <MultiSelectComboBox
                options={studentOptions}
                value={selectedStudentIds}
                onChange={setSelectedStudentIds}
                placeholder={studentsLoading ? "در حال بارگذاری..." : "جستجو و انتخاب قرآن آموزان..."}
                disabled={studentsLoading}
              />
            </div>
            <div>
              <label className="block text-sm font-semibold text-zinc-700 dark:text-zinc-300 mb-1.5 text-right">از تاریخ</label>
              <DateSelector selectedDate={fromDate} onChange={setFromDate} />
            </div>
            <div>
              <label className="block text-sm font-semibold text-zinc-700 dark:text-zinc-300 mb-1.5 text-right">تا تاریخ</label>
              ...
              <table className="w-full text-sm border-collapse">
                <thead>
                  <tr className="bg-zinc-50 dark:bg-zinc-800/50 border-b border-zinc-100 dark:border-zinc-800">
                    ...
                    {days.map((day) => (
                      <th key={day.dateStr} className="text-center px-2 py-3 font-semibold text-zinc-600 dark:text-zinc-300 min-w-[90px]">
                        <div className="text-xs">{dayLabels[day.dayOfWeek]}</div>
                  ...
                  {selectedStudents.map((student) => (
                    <tr key={student.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/30 transition-colors">
                      ...
                      {days.map((day) => {
                        ...
                        const done = progressMap[student.id]?.[day.dateStr] || [];
                        const allDone = val.length > 0 && val.every(j => done.includes(j));
                        const someDone = val.length > 0 && done.some(j => val.includes(j));
                        return (
                          <td key={day.dateStr} className="px-2 py-2.5 text-center align-middle">
                            <div className="flex items-center justify-center">
                              <div className="relative">
                                ...
                                <JuzMultiSelectCell
                                  value={val}
                                  onChange={(next) => setCellValue(student.id, day.dateStr, next)}
                                />
                              </div>
                            </div>
                            {allDone && (
                              <div className="text-[10px] text-emerald-600 dark:text-emerald-400 mt-0.5">تکمیل</div>
                            )}
                            {someDone && !allDone && (
            ...
            <div className="flex items-center justify-between px-4 py-3 border-t border-zinc-100 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-800/50">
              <div className="flex items-center gap-2">
                <span className="text-sm text-zinc-500">
                  {selectedStudentIds.length} قرآن آموز × {days.length} روز = {totalAssignments} تکلیف
                </span>
              </div>
              <div className="flex items-center gap-2">
                <Button variant="outline" onClick={handleLoad} disabled={loadingData || selectedStudentIds.length === 0 || days.length === 0}>
                  {loadingData ? <Loader2 className="ml-2 h-4 w-4 animate-spin" /> : <Download className="ml-2 h-4 w-4" />}
                  بارگذاری برنامه
                </Button>
                <Button onClick={handleSave} disabled={saving || totalAssignments === 0}>
        ...
        {selectedStudentIds.length === 0 && (
          <Card className="p-12 bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 rounded-xl shadow-md">
            <p className="text-center text-zinc-400">قرآن آموزان و بازه زمانی را انتخاب کنید</p>
          </Card>
        )}
      </div>
    </PageTransition>
