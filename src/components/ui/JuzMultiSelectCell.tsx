    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button
          type="button"
          className={cn(
            "w-full min-h-[36px] sm:h-9 px-2 py-1.5 sm:py-0 rounded-md border text-xs flex items-center justify-center gap-1 text-center transition-colors",
          ...
          {hasValue ? (
            <span className="flex flex-wrap items-center justify-center gap-1 leading-none">
              {sorted.map((n) => (
                <span
                  key={n}
                  className="inline-flex items-center justify-center min-w-[22px] h-6 sm:min-w-[20px] sm:h-5 px-1.5 sm:px-1 rounded bg-emerald-100 dark:bg-emerald-900/40 text-emerald-800 dark:text-emerald-200 text-xs sm:text-[11px] font-semibold"
                >
                  {n}
                </span>
              ))}
      ...
      <PopoverContent
        align="center"
        sideOffset={4}
        className="w-72 sm:w-64 p-3 sm:p-2 max-h-80 overflow-auto"
      >
          ...
          {Array.from({ length: max }, (_, i) => i + 1).map((n) => {
            ...
            return (
              <label
                key={n}
                className={cn(
                  "flex items-center gap-1.5 px-2 py-2 sm:py-1.5 rounded cursor-pointer text-xs transition-colors",
              ...
              >
                ...
                {checked && (
                  <Check className="h-3.5 w-3.5 sm:h-3 sm:w-3 text-emerald-600 dark:text-emerald-400" />
                )}
              </label>
            );
          })}
